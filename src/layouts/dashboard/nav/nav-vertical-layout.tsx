...
export function NavVerticalLayout({ data, className }: Props) {
	...
	return (
		<nav
			...
			className={cn(
				"fixed inset-y-0 right-0 flex-col h-full bg-background border-l border-dashed z-app-bar transition-[width] duration-300 ease-in-out",
				className,
			)}
		>
			<div
			...
			>
				<div className="flex items-center gap-2">
					<Logo />
					{themeLayout !== ThemeLayout.Mini && <span className="text-xl font-bold text-primary">نرم افزار کنترلر</span>}
				</div>
			</div>
