...
const MainTable: React.FC<MainTableProps> = ({ dataSource, selectedRow, onRowSelect, title }) => {
	const tableRef = useRef<HTMLDivElement>(null);

	return (
		<div className="rounded-xl border shadow-sm overflow-hidden">
			<div className="w-full flex justify-between  items-center">
				<div className="w-full flex  px-2  font-bold items-center justify-start gap-3">{title}</div>
				<PrintData
					dataExcel={{
						data: dataSource,
