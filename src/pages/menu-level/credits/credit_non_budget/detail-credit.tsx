				...
				<>
					...
					>
						...
						<div className="grid grid-cols-4 gap-4 mb-6 text-right font-semibold text-sm border-b pb-2">
							<div>شماره برگه اعتبار: {/* <span className="font-normal">{data || "-"}</span> */}</div>
							<div>تاریخ برگه اعتبار: {/* <span className="font-normal">{data.creditDate || "-"}</span> */}</div>
							<div>
								مبلغ: <span className="font-normal">{/* {data.amount?.toLocaleString() || "-"} */}</span>
							</div>
							<div>کد یگان واگذار شده: {/* <span className="font-normal">{data.unit || "-"}</span> */}</div>
						</div>
