...
const TankhahTable: React.FC<TankhahTableProps> = ({ selectedRow }) => {
	...
	return (
		<div className="bg-muted/30 border rounded-xl shadow-sm p-4 space-y-2">
			<h3 className="font-semibold text-base text-gray-700 mb-2">لیست تنخواه پرداختی</h3>
			<PrintData
				dataExcel={{
					data: [],
			...
			<div ref={tableRefTankhah} className="w-full">
				<Table rowKey={(_, index) => `tankhah-${index}`} size="small" columns={tankhahTableColumns} dataSource={[]} pagination={false} />
				<div className="text-sm font-semibold text-right pt-2 border-t mt-2">جمع کل تنخواه: ۰ ریال</div>
			</div>
		</div>
	);
};
