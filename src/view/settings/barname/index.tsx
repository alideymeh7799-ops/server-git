...
export default function BARNAMEManager() {
  ...
  useEffect(() => {
    let isMounted = true;
    if (isMounted && can(resourceList.BARNAME, operationList.GETALL)) {
      getData.fetchData();
    }
    return () => {
      isMounted = false;
  ...
  const sendCreate = axiosHook.usePostData(
    buildApiRoute(resourceList.BARNAME, operationList.CREATE)
  );
  // !!________UPDATE DATA
  const sendUpdate = axiosHook.useUpdateData(
    buildApiRoute(
      resourceList.BARNAME,
  ...
  const sendDelete = axiosHook.useDeleteData(
    buildApiRoute(resourceList.BARNAME, operationList.DELETE)
  );
  const closeModal = () => {
      ...
      <CardHeader>
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
          ...
          <div className="flex gap-2">
            {can(resourceList.BARNAME, operationList.SEARCH) && (
              <Input
                placeholder="جستجو..."
                value={search}
            ...
            {can(resourceList.BARNAME, operationList.CREATE) && (
              <Button
